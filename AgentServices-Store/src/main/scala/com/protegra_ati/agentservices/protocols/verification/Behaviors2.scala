...
case class ReliantPartyBehavior2() {

  def run(kvdbNode: KVDBNode, c2r: Connection) {
    implicit val kvdb = kvdbNode

    // Wait for claim.
    Tweet("Reliant Party: Waiting for SubmitClaim")
    c2r ? ({ case SubmitClaim(token, claimStr, c2v, c2r) =>
      BasicLogService.tweet("Reliant Party: Received a SubmitClaim")
      ...
      BasicLogService.tweet("Reliant Party: Sending ProduceClaim")
      r2v ! ProduceClaim(token, claimStr, r2c, r2v)
      BasicLogService.tweet("Reliant Party: Sent ProduceClaim")

      // Wait for the verifier
      val v2c = claim.connection(Verifier, Claimant)
      val v2r = claim.connection(Verifier, RelParty)

      BasicLogService.tweet("Reliant Party: Waiting for ValidateClaim")
      v2r ? ({ case ValidateClaim(tokenPrime, claimStrPrime, response, v2cPrime, v2rPrime) =>
        BasicLogService.tweet("Reliant Party: Received ValidateClaim")

        // TODO: Notify the user.
        BasicLogService.tweet("\n\nClaim Verified.\n\n")

        // Notify the claimant
        BasicLogService.tweet("Reliant Party: Sending CompleteClaim")
        r2c ! CompleteClaim(token, claimStr, r2c, r2v)
        BasicLogService.tweet("Reliant Party: Sent CompleteClaim")
...
object Behaviors2 {
  ...
  val verifier = VerifierBehavior2()
  val reliantParty = ReliantPartyBehavior2()
  val claimant = ClaimantBehavior2()
